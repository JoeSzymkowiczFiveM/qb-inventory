...
function openContextMenu(e, args) {
    ...
    $.each(args, function (index, value) {
        if (value.status !== "blur(5px)") {
            $(".contextmenu").prepend(
                `
                <li class="contextmenu-item ${value.className}" data-info="${value.info}" data-status="${value.status}" data-fromslot="${value.fromSlot}" data-frominventory="${value.fromInventory}" data-itemdata="${value.itemData}"  >
...
function embedImage(text) {
    if ( /(http[s]*:\/\/)([a-z\-_0-9\/.]+)\.([a-z.]{2,3})\/([a-z0-9\-_\/._~:?#\[\]@!$&'()*+,;=%]*)([a-z0-9]+\.)(jpg|jpeg|png)/.test(text) ) {
        text = '<img src=' + text + ' />'
    } else {
        text = '<img src="images/' + text + '" />'
    }
    return text
}
...
$(document).on('mousedown', '.item-slot', function(event) {
    switch (event.which) {
        case 3:
            contextData.fromInventory = $(this).parent().attr("data-inventory")
            let options = []
            fromSlot = $(this).attr("data-slot");
            ItemData = $(this).data("item");
            contextData.fromSlot = fromSlot
            contextData.itemData = ItemData
            ClickedItemData = ItemData
            if (ItemData !== null && ItemData !== undefined && ItemData.name !== undefined) {
                if (contextData.fromInventory === 'player') {
                    ...
                    if (ItemData.info.serie) {
                        options.push({
                            className: "copy-serie",
                            icon: "fa-regular fa-copy",
                            text: "Copy Serial",
                        })
                    }

                    if (ItemData.useable == true) {
                        options.push({
                            className: "use-item",
                ...
                } else if (contextData.fromInventory === 'crafting') {
                    options.push({
...
$(".contextmenu").on("click", ".use-item", function () {
    // $.post(
    ...
    //const Elem = $(this).data("frominventory");
    //console.log(Elem)
    //fromData = contextData.itemData;
    //fromInventory = contextData.fromInventory;
    amount = $("#item-amount").val();
    if (amount == 0) { amount = contextData.itemData.amount }
    if (contextData.itemData) {
        Inventory.Close();
        $.post(
            "https://qb-inventory/UseItem",
            JSON.stringify({
                inventory: contextData.fromInventory,
                item: contextData.itemData,
            })
        );
    }
});

$(".contextmenu").on("click", ".craft-item", function () {
    ...
    if (amount === '0') {
        InventoryError($(".other-inventory"), contextData.fromSlot);
        return;
    }
    if (contextData.itemData) {
        Inventory.Close();
...
$(".contextmenu").on("click", ".copy-serie", function () {
    const el = document.createElement('textarea');
    el.value = contextData.itemData.info.serie;
...
$(".contextmenu").on("click", ".modify-item", function () {
    if (contextData.itemData) {
        Inventory.Close();
        $.post(
            "https://qb-inventory/ModifyItem",
            JSON.stringify({
                name: contextData.itemData.name,
                slot: contextData.itemData.slot,
            })
        );
    }
});

$(".contextmenu").on("click", ".attachments-menu", function () {
    if (!Inventory.IsWeaponBlocked(contextData.itemData.name)) {
        ...
        FormatAttachmentInfo(contextData.itemData);
    } else {
...
$(".contextmenu").on("click", ".combine-item", function (e) {
    e.preventDefault();
...
$(".contextmenu").on("click", ".drop-item", function () {
    fromSlot = contextData.fromSlot
    if (contextData.fromInventory == "player") {
        toInventory = $(".other-inventory");
        ...
        toInventory = $(".player-inventory");
        fromInventory = $(".other-inventory");
    }
    toSlot = GetFirstFreeSlot(toInventory, $(this));
    if (contextData.itemData === undefined) {
        return;
    }
    toAmount = $("#item-amount").val();
    if (toAmount == 0) { toAmount = contextData.itemData.amount }
    //toAmount = contextData.itemData.amount;
    if (ControlPressed) {
        if (toAmount > 1) {
            toAmount = Math.round(toAmount / 2)
        }
...
function FormatAttachmentInfo(data) {
    $.post("https://qb-inventory/GetWeaponData", JSON.stringify({
        ...
        if (data.WeaponData.ammotype == "AMMO_RIFLE") {
            ...
            AmmoLabel = "12 Gauge"
        } else if (data.WeaponData.ammotype == "AMMO_SMG") {
            AmmoLabel = "5.7"
        }
        ...
        if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
            if (data.AttachmentData.length > 0) {
                ...
                $.each(data.AttachmentData, function(i, attachment) {
                    ...
                    $(".weapon-attachments").append('<div class="weapon-attachment" id="weapon-attachment-' + i + '"> <div class="weapon-attachment-label"><p>' + attachment.label + '</p></div> <div class="weapon-attachment-img"><img src="./images/' + WeaponType + '_' + attachment.attachment + '.webp"></div> </div>')
                    attachment.id = i;
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
        ...
        } else if (itemData.name == "lspd_badge") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Officer </strong><span>' + itemData.info.lastname + '\'s badge</span></p><p><strong>Callsign: </strong>#<span>' + itemData.info.callsign + '</span></p></p>');
        } else if (itemData.name == "weapons_license") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>First Name: </strong><span>' + itemData.info.firstname + '</span></p><p><strong>Last Name: </strong><span>' + itemData.info.lastname + '</span></p><p><strong>Birth Date: </strong><span>' + itemData.info.birthdate + '</span></p><p><strong>Licenses: </strong><span>' + itemData.info.type + '</span></p>');
        } else if (itemData.name == "pilots_license") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>First Name: </strong><span>' + itemData.info.firstname + '</span></p><p><strong>Last Name: </strong><span>' + itemData.info.lastname + '</span></p><p><strong>Birth Date: </strong><span>' + itemData.info.birthdate + '</span></p><p><strong>Licenses: </strong><span>' + itemData.info.type + '</span></p>');
            ...
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<img src="' + itemData.info.url + '" alt="' + itemData.name + '" style="display:block;margin-left:auto;margin-right:auto;" width="200rem" height="200rem"/></div>');
            ...
            if (itemData.info.attachments != null) {
                ...
                $(".item-info-description").html('<p><strong>Serial Number: </strong><span>' + itemData.info.serie + '</span></p><p><strong>Loaded Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p><strong>Attachments: </strong><span>' + attachmentString + '</span></p>');
            } else {
                $(".item-info-description").html('<p><strong>Serial Number: </strong><span>' + itemData.info.serie + '</span></p><p><strong>Loaded Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p>' + itemData.description + '</p>');
            }
        ...
        } else if (itemData.name == "methlabkey") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Lab: </strong><span>' + itemData.info.lab + '</span></p><p><strong>Location: </strong><span>' + itemData.info.location + '</span></p>');
        } else if (itemData.name == "cokelabkey") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Lab: </strong><span>' + itemData.info.lab + '</span></p><p><strong>Location: </strong><span>' + itemData.info.location + '</span></p>');
        } else if (itemData.name == "license_plate") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Plate: </strong><span>' + itemData.info.plate + '</span></p>');
        } else if (itemData.name == "meth") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Purity: </strong><span>' + itemData.info.purity + '</span></p>');
        } else if (itemData.name == "coke") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Purity: </strong><span>' + itemData.info.purity + '</span></p>');
        } else if (itemData.name == "rental_papers") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Name: </strong><span>' + itemData.info.firstname + ' ' + itemData.info.lastname + '</span></p><p><strong>Plate: </strong><span>' + itemData.info.plate + '</span></p><p><strong>Model: </strong><span>' + itemData.info.vehicle + '</span></p><p><strong>Date: </strong><span>' + itemData.info.date + '</span></p>');
        } else if (itemData.name == "usb_orange") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Amount: </strong><span>' + "&#x20BF; " + itemData.info.cryptoamount + '</span></p><p><span>' + itemData.description + '</span></p>');
        } else if (itemData.name == "backpack_small" || itemData.name == "backpack_medium" || itemData.name == "backpack_large") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html("<p><strong>Backpack: </strong><span>" + itemData.info.serie + "</span></p><p>" + itemData.description + "</p>" );
        ...
        } else if (itemData.name.includes("pokemon_card")) {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Rarity: </strong><span>'+ itemData.info.rarity + '</span></p>');
        } else if (itemData.name.includes("vehicle_keys")) {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Plate: </strong><span>' + itemData.info.plate + '</span></p><p><strong>Brand: </strong><span>' + itemData.info.make + '</span></p><p><strong>Model: </strong><span>' + itemData.info.model + '</span></p></p>');
        } else {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
...
function handleDragDrop() {
    ...
    $(".item-slot").droppable({
        ...
        drop: function(event, ui) {
            ...
            itemData = fromInventory.find("[data-slot=" + fromSlot + "]").data("item");
            if (toSlot == 41 && itemData.name != 'vehicle_keys'){
                InventoryError(fromInventory, fromSlot);
                $.post('https://qb-inventory/Notify', JSON.stringify({
                    message: "This slot is only for Vehicle Keys.",
                    type: "error"
                }))
                return false;
            }
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if ($toInv.attr("data-inventory").split("-")[0] == "itemshop" || $toInv.attr("data-inventory") == "crafting") {
        ...
        if ($toInv.attr("data-inventory").split("-")[0] == "itemshop") {
            //this break inventory when trying to move items back into crafting or itemshop, fix this hsit
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData.created === undefined) { //this is a bit hacky solution for stores items
        fromData.created = [];
        for(i = 0; i < $toAmount; i++){
            fromData.created.push(Date.now());
        };
    }
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if ((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            newData.degrade = toData.degrade;
            //newData.quality = toData.quality;

            if (fromData.amount == $toAmount) {
                //console.log("1.1")
                var newDataFromCreated = toData.created.concat(fromData.created).sort(function(a,b){return a - b})
                newData.created = newDataFromCreated;
                $toInv.find("[data-slot=" + $toSlot + "]").data("item", newData);

                $toInv.find("[data-slot=" + $toSlot + "]").addClass("item-drag");
                $toInv.find("[data-slot=" + $toSlot + "]").removeClass("item-nodrag");

                var ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + newData.label + '</p></div>';
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    ...
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + newData.image + '" alt="' + newData.name + '" /></div><div class="item-slot-amount"><p>' + newData.amount + ' (' + ((newData.weight * newData.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                } else {
                ...
                newData.info.quality = ItemQualityAdjustment(newData.created, newData.degrade)
                //Inventory.QualityCheck(newData, false, false);
                ...
                //if ((newData.name).split("_")[0] == "weapon") {
                    //if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        if (newData.info.quality !== undefined) {
                            qualityLabel = Math.ceil(newData.info.quality);
                        } else {
                ...
                newDataFrom.degrade = fromData.degrade;
                newDataFrom.slot = parseInt($fromSlot);
                newDataFrom.created = fromData.created.slice(parseInt($toAmount));
                newDataFrom.info.quality = ItemQualityAdjustment(newDataFrom.created, newDataFrom.degrade);
                ...
                if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
                    ...
                    var ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + newDataFrom.label + '</p></div>';
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        ...
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>' + newDataFrom.amount + ' (' + ((newDataFrom.weight * newDataFrom.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    } else {
                    ...
                    if (newDataFrom.info.quality !== undefined) {
                        qualityLabel = Math.ceil(newDataFrom.info.quality);
                    } else {
                ...
                newData.created = newData.created.concat(fromData.created.slice(0, parseInt($toAmount)));
                newData.info.quality = ItemQualityAdjustment(newData.created, newData.degrade);
            ...
            //console.log("2")
            if (fromData.amount == $toAmount) {
                if (fromData.info.type === 'bag' && $toInv.attr("data-inventory").includes("stash-backpack")) { //prevents putting bags in bags
                    InventoryError($fromInv, $fromSlot);
                    return false;
                }
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    ...
                    const status = $(this).data("type");
                    contextData.fromSlot = $fromSlot
                    contextData.fromInventory = $fromInv.attr("data-inventory")
                    contextData.itemData = fromData
                    openContextMenu(event, options);
                    return
                }
                ...
                var ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + fromData.label + '</p></div>';
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    ...
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + ' (' + ((fromData.weight * fromData.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                } else {
                ...
                fromData.info.quality = ItemQualityAdjustment(fromData.created, fromData.degrade)
                ...
                //if ((fromData.name).split("_")[0] == "weapon") {
                    //if (!Inventory.IsWeaponBlocked(fromData.name)) {
                        ...
                        if (fromData.info.quality !== undefined) {
                            qualityLabel = Math.ceil(fromData.info.quality);
                        } else {
                ...
                if (toData != undefined) {
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        ...
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + ' (' + ((toData.weight * toData.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    } else {
                    ...
                    toData.info.quality = ItemQualityAdjustment(toData.created, toData.degrade)
                    ...
                    //if ((toData.name).split("_")[0] == "weapon") {
                        //if (!Inventory.IsWeaponBlocked(toData.name)) {
                            ...
                            if (toData.info.quality !== undefined) {
                                qualityLabel = Math.ceil(toData.info.quality);
                            } else {
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        ...
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div>');
                    } else {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div>');
                    }
                ...
                newDataTo.degrade = fromData.degrade;
                newDataTo.slot = parseInt($toSlot);
                newDataTo.created = fromData.created.slice(0, parseInt($toAmount))
                newDataTo.info.quality = ItemQualityAdjustment(newDataTo.created, fromData.degrade)
                ...
                var ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + newDataTo.label + '</p></div>';
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    ...
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + newDataTo.image + '" alt="' + newDataTo.name + '" /></div><div class="item-slot-amount"><p>' + newDataTo.amount + ' (' + ((newDataTo.weight * newDataTo.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                } else {
                ...
                //if ((newDataTo.name).split("_")[0] == "weapon") {
                    //if (!Inventory.IsWeaponBlocked(newDataTo.name)) {
                        ...
                        var QualityColor = "rgb(39, 174, 96)";
                        if (newDataTo.info.quality >= 50) {
                            QualityColor = "rgb(39, 174, 96)";
                        } else if (newDataTo.info.quality > 25) {
                            QualityColor = "rgb(230, 126, 34)";
                        }
                        else if (newDataTo.info.quality < 25) {
                            QualityColor = "rgb(192, 57, 43)";
                        ...
                        if (newDataTo.info.quality !== undefined) {
                            qualityLabel = Math.ceil(newDataTo.info.quality);
                        } else {
                ...
                //console.log("2.2.2")
                //console.log(fromData.created)
                //console.log("fromData.degrade", fromData.degrade)
                var newDataFrom = [];
                newDataFrom.name = fromData.name;
                newDataFrom.label = fromData.label;
                newDataFrom.amount = parseInt((fromData.amount - $toAmount));
                ...
                newDataFrom.unique = fromData.unique;
                newDataFrom.slot = parseInt($fromSlot);
                newDataFrom.created = fromData.created ? fromData.created.slice(parseInt($toAmount)) : Array(newDataFrom.amount).fill(Date.now())
                newDataFrom.info.quality = ItemQualityAdjustment(newDataFrom.created, fromData.degrade)
                //console.log(newDataFrom.info.quality)
                ...
                var ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + newDataFrom.label + '</p></div>';
                if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
                    $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>(' + newDataFrom.amount + ') $' + newDataFrom.price + '</p></div>' + ItemLabel);
                } else {
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>' + $fromSlot + '</p></div><div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>' + newDataFrom.amount + ' (' + ((newDataFrom.weight * newDataFrom.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    } else if ($fromSlot == 41 && $fromInv.attr("data-inventory") == "player") {
                ...
                //console.log("This is dragging an amount/item from another item in player inv")
            } else {
                InventoryError($fromInv, $fromSlot);
            }
        }
...
(() => {
        ...
        //if (!Inventory.IsWeaponBlocked(item.name)) {
            //if ((item.name).split("_")[0] == "weapon") {
                ...
                if (item.info.quality !== undefined) {
                    qualityLabel = Math.ceil(item.info.quality);
                } else {
    ...
    Inventory.Open = function(data) {
        ...
        for (i = 6; i < (data.slots + 1); i++) {
            if (i == 41) {
                $(".player-inventory").append('<div class="item-slot" data-slot="' + i + '"><div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"></div><div class="item-slot-label"><p>&nbsp;</p></div></div>');
            } else {
            ...
            $.each(data.inventory, function(i, item) {
                if (item != null) {
                    ...
                    if (item.slot < 6) {
                        ...
                        $(".player-inventory").find("[data-slot=" + item.slot + "]").addClass("item-drag");
                        $(".player-inventory").find("[data-slot=" + item.slot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                        $(".player-inventory").find("[data-slot=" + item.slot + "]").data("item", item);
                    } else {
                        $(".player-inventory").find("[data-slot=" + item.slot + "]").addClass("item-drag");
                    ...
                    item.info.quality = ItemQualityAdjustment(item.created, item.degrade)
    ...
    Inventory.Update = function(data) {
        ...
        $.each(data.inventory, function(i, item) {
            if (item != null) {
                ...
                if (item.slot < 6) {
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").addClass("item-drag");
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").html('<div class="item-slot-key"><p>' + item.slot + '</p></div><div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div><div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + item.label + '</p></div>');
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").data("item", item);
                } else if (item.slot == 41) {
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").addClass("item-drag");
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").html('<div class="item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div><div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + item.label + '</p></div>');
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").data("item", item);
                } else {
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").addClass("item-drag");
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").html('<div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div><div class="item-slot-quality"><div class="item-slot-quality-bar"><p>100</p></div></div><div class="item-slot-label"><p>' + item.label + '</p></div>');
                    $(".player-inventory").find("[data-slot=" + item.slot + "]").data("item", item);
                }
                item.info.quality = ItemQualityAdjustment(item.created, item.degrade)
                Inventory.QualityCheck(item, false, false);
    ...
    Inventory.ToggleHotbar = function(data) {
        if (data.open) {
            ...
            //var elem = '<div class="z-hotbar-item-slot" data-zhotbarslot="41"> <div class="z-hotbar-item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>'
            //$(".z-hotbar-inventory").append(elem);
            $.each(data.items, function(i, item) {
                if (item != null) {
                    ...
                    if (item.slot == 41) {
                        $(".z-hotbar-inventory").find("[data-zhotbarslot=" + item.slot + "]").html('<div class="z-hotbar-item-slot-key"><p>L <i class="fa-solid fa-key"></i></p></div><div class="z-hotbar-item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="z-hotbar-item-slot-amount"><p>' + item.amount + ' (' + ((item.weight * item.amount) / 1000).toFixed(1) + ')</p></div>' + ItemLabel);
                    } else {
        ...
        if (data.toggle) {
            if (!requiredItemOpen) {
                ...
                $.each(data.items, function(index, item) {
                    var element = '<div class="requiredItem-box"><div id="requiredItem-action">Requires</div><div id="requiredItem-label"><p>' + item.label + '</p></div><div id="requiredItem-image"><div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div></div></div>'
                    $(".requiredItem-container").hide();
...
function ItemQualityAdjustment(time, degrade) {
    ...
    const TimeExtra = 2419200 * DecayRate;
    const StartDate = time * 1000;
    const dateNow = Date.now();
    
    var quality = 100 + (100 - ((dateNow - StartDate) / TimeExtra) * 100) / 1000;
    if (quality > 100) { quality = 100 } else if (quality < 0.1) { quality = 0 }
    return quality;
}